...
// Maybe no need to initialize the 2D arrays
// Use valarray and better 2D arrays or even use contiguous arrays
// (1d, not 2D)
...
#define NULL_DELETE_ARRAY(pointer) {                              \
 ...
 #define PR(x);
  ...
  void delete_2(double **x, int nrow) {// delete the 2-D array
    int i;
    for (i = 0; i < nrow; ++i) {
      NULL_DELETE_ARRAY(x[i]);
      // delete [] x[i];
      // x[i] = NULL;
    }
    NULL_DELETE_ARRAY(x);
							 ...
							 double *loglik)
  {
	...
	if (*k==1) { // with one state is just log lik.
	  ...
	  double *filter = new double[*k]; 
	  double *filtercond = new double[*k];
	
	  for (int hidden_state=0; hidden_state < *k; ++hidden_state) {
	  ...
	  for (int obs_index = 0; obs_index < *n; ++obs_index) {
		...
		if (obs_index == 11) printf("\n x_11:  %f", x_i); //a partir aquí, problema
		...
		for (int hidden_state = 0; hidden_state <*k; ++hidden_state) {
		  rowSumsQ = 0;
		  for(int hidden_state_b = 0; hidden_state_b < *k; ++hidden_state_b) {
...
// 			Q[hidden_state][hidden_state_b] = Q1 * Q2;
  ...
  void Birth(double *y, int *genome, int *index, double *mu, double *sigma2, 
	     double *beta, double *stat,
	     double *statBirth, double *ka, double *g,
    ...
    double *candidatoQ = new double[(*r+1)*(*r+1)];
    double probBirth;
    int newState;
    ...
    candidatoSigma2[*r] = 1 / rgamma(*ka, *g);
  ...
  void Split(double *y, int *genome, int *index, double *mu, double *sigma2, 
	     double *beta, 
	     double *stat, double *statSplit, double *ka, double *g, 
    ...
    int newState;
    double *ui = new double[*r-1];
    double *epj = new double[*r-1];
    double epMu, epSigma2;
    double *gi0 = new double[2];
    double jacobian;
    int i, j, k;
    double loglikCandidate = 0;
    double loglikPartial = 0;
	...
	for (j=0; j< (split-1); ++j) {
	  ASSERT(indexBeta < ((*r + 1) * (*r + 1)), 
			 "Size of indexBeta out of bounds" );
	  candidatoBeta[indexBeta] = beta[i* *r + j];
	  candidatoQ[indexBeta] = -candidatoBeta[indexBeta++];
	}
    ...
    candidatoMu[indexMu] = mu[split-1] - sqrt(sigma2[split-1]) * epMu;
	...
	probSplit = probSplit - dbeta(ui[i], 2, 2, 1);
      ...
      jacobian =*r * log(2) + 3 * log(sqrt(sigma2[split-1]));
      if (split > 1) {
	for (i=0; i< split-1; ++i) {
  ...
  void Combine(double *y, int *genome, int *index, double *mu, double *sigma2, 
	       double *beta, double *stat, double *statCombine, double *ka, double *g,
	       int *r, double *loglikLast, double *probK, double *ps, 
	       double *x, int *n, double *candidatoMu,
	       double *candidatoSigma2, 
	...
	for (i=0; i<*r-2; ++i) {
      ...
      // jacobian of the transformation
      // r-2 because of a 2 in the denominator and the split equivalent is r-1
      jacobian = (*r-1) * log(2) + 3* log(sqrt(sigma2[combine-1]));
      if (combine > 1) {
	for (i=0; i< combine-1; ++i) {
  ...
  void MetropolisUpdate(double *y, double *x, int *genome, int *index, double *mu, 
			double *sigma2, double *beta, double *stat, int *r, int *n, 
			double *muAlfa, double *muBeta, double *ka, 
			double *g, double *sigmaTauMu,
    ...
    double priorSigma2, priorCandidatoSigma2;
    double *candidatoMu = new double[*r];
    double *candidatoSigma2 = new double[*r];
    double *candidatoBeta = new double[*r * *r];
    int reachedMaxVar=0;
    int nn;
    //update mu
	...
	*loglikLast = loglikCandidate;
	...
	dinvgamma(&sigma2[i], ka, g, 1, &priorSigma2);
	dinvgamma(&candidatoSigma2[i], ka, g, 1, &priorCandidatoSigma2);
	acepProb = acepProb + priorCandidatoSigma2 - priorSigma2;
      }
      for (k=0; k < *genome; ++k) {
	nn = index[k+1] - index[k];
	double *yy = new double[nn];
  ...
  void viterbi(double *y, double *x, int *genome, int *index, 
    ...
    for (g=0; g < *genome; ++g) {
      ...
      double *yy = new double[nn];
      double *xx = new double [nn];
      double *m = new double[nn* *k];
      int *b = new int[nn* *k];
      int *bAux = new int[*k];
      double *mAux= new double[*k];
      for (i=0; i < nn-1; ++i) {
	yy[i] = y[i + index[g]];
	xx[i] = x[i + index[g]];
      }
  ...
  void MetropolisSweep(double *y, double *x, int *genome, int *index, 
		       int *kMax, int *n, int *burnin,
    ...
    // Verify we can run
    unsigned long int max_int = static_cast<unsigned long int>(std::numeric_limits<int>::max());
    ...
    double *loglikLast = new double[*kMax];
    //index to permutations
    int *indexPerm = new int[*kMax];
    //index to start every object
    int *indexMu = new int[*kMax]; //same for mu and sigma2
    int *indexBeta = new int[*kMax];
    int *indexStat = new int[*kMax];
    //old parameters (max limit)
    double *OldStat = new double[*kMax];
    double *OldMu= new double[*kMax];
    double *OldSigma2= new double[*kMax];
    double *OldBeta= new double[*kMax * *kMax];
    //new parameters (max limit)

    double *NewStat = new double[*kMax];
    double *NewMu= new double[*kMax];
    double *NewSigma2= new double[*kMax];
    double *NewBeta= new double[*kMax * *kMax];
    
    //auxiliaries for viterbi
    double *AuxSigma2= new double[*kMax];
    double *AuxBeta= new double[*kMax * *kMax];

    double *q = new double[*kMax * *kMax];
    ...
    *accepted = 0;
    ...
    // do this appropriately: fill with true junk zz
    for(int junkindex = 0; junkindex < ((*kMax) * (*kMax)); ++junkindex) {
      q[junkindex] = -99;
      OldBeta[junkindex] = -99;
      NewBeta[junkindex] = -99;
      AuxBeta[junkindex] = -99;
    }
    for(int junkindex = 0; junkindex < (*kMax) ; ++junkindex) {
...
//              #endif
	...
	OldBeta[i] = beta[indexBeta[r-1] + ((times[r-1]-1)*r*r) + i];
      ...
      PR(r-1);
      PR(sigmaTauMu[r-1]);
      MetropolisUpdate(y, x, genome, index, OldMu, OldSigma2, OldBeta, OldStat, &r, 
		       n, muAlfa, muBeta, ka, g, &sigmaTauMu[r-1], &sigmaTauSigma2[r-1], 
	...
	if(runif(0,1) <= pb[r-1]) {
	  ...
	  Birth(y, genome, index, OldMu, OldSigma2, OldBeta, OldStat, NewStat, 
		ka, g, &r, &loglikLast[r-1], probK,
	...
	if(runif(0,1) <= ps[r-1]) {
	  ...
	  Split(y, genome, index, OldMu, OldSigma2, OldBeta, OldStat, NewStat, 
		ka, g, &r, &loglikLast[r-1], probK, ps, x, n, NewMu, 
	  ...
	  Combine(y, genome, index, OldMu, OldSigma2, OldBeta, OldStat, NewStat, 
		  ka, g, &r, &loglikLast[r-1], probK, ps, x, n, NewMu, 
